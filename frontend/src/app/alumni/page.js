...
export default function AlumniAngelsPage() {
  ...
  return (
    <div className="page-shell flex flex-col overflow-hidden">
      <header className="page-header sticky top-0 z-40 px-5 py-5">
        <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-amber-300 to-[#D4A843] text-black shadow-lg shadow-amber-500/20">
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 overflow-y-auto px-4 py-6 sm:px-5 sm:py-8">
        {alumniList.length > 0 ? (
            ...
            {alumniList.map(alumni => (
              ...
              >
                ...
                <div className="relative mb-4 sm:mb-0 sm:mr-6 shrink-0">
                  <img src={alumni.image} className="w-20 h-20 rounded-2xl object-cover border-2 border-[#E8E6E0]" alt={alumni.name} />
                ...
                <div className="flex-1 text-center sm:text-left w-full">
                  ...
                  <div className="flex flex-col sm:flex-row items-center sm:items-start space-y-2 sm:space-y-0 sm:space-x-3 mb-5">
                    <div className="flex items-center bg-[#F3F2EE] border border-[#E8E6E0] px-3 py-1.5 rounded-lg">
                      ...
                      <span className="text-xs font-bold text-[#1A1A1A]">{alumni.company}</span>
                    </div>
                    <div className="flex items-center bg-[#F3F2EE] border border-[#E8E6E0] px-3 py-1.5 rounded-lg">
                      <Briefcase size={14} className="text-amber-400 mr-2" />
                      <span className="text-xs font-bold text-[#4A4A4A]">{alumni.role}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex space-x-3 w-full">
                    <button 
                      ...
                      className="flex-1 py-3 bg-gradient-to-r from-[#C8922A] to-[#D4A843] text-black rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-amber-500/20 hover:shadow-amber-500/40 transition flex items-center justify-center disabled:opacity-50 disabled:grayscale disabled:cursor-not-allowed"
                    >
                      <Coffee size={14} className="mr-2" /> Coffee Chat
                    </button>
                    <button className="px-4 py-3 bg-[#F3F2EE] hover:bg-[#F3F2EE] border border-[#E8E6E0] rounded-xl text-[#1A1A1A] transition flex items-center justify-center">
          ...
          <div className="app-panel mx-auto flex min-h-[420px] max-w-2xl flex-col items-center justify-center rounded-[1.75rem] p-8 text-center">
            ...
            <h3 className="text-2xl font-black tracking-tight text-[#1A1A1A]">Alumni network is warming up</h3>
            ...
            <div className="mt-6 grid w-full max-w-md grid-cols-1 gap-3 min-[420px]:grid-cols-3">
              {[
                ...
                <div key={label} className="rounded-2xl border border-[#E8E6E0] bg-[#F3F2EE] p-4">
      ...
      <AnimatePresence>
        {selectedAlumni && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setSelectedAlumni(null)}>
            <motion.div 
            ...
            >
              <div className="p-6 border-b border-[#E8E6E0] text-center">
                <Coffee size={32} className="text-amber-500 mx-auto mb-3" />
                <h3 className="text-lg font-black text-[#1A1A1A] uppercase tracking-wider">Request Coffee Chat</h3>
                <p className="text-xs text-[#6B6B6B]">with {selectedAlumni.name} ({selectedAlumni.company})</p>
              </div>
              <div className="p-6 space-y-4">
                <div>
                  <label className="text-[10px] text-[#6B6B6B] font-black uppercase tracking-widest block mb-2">Topic of Discussion</label>
                  <select className="w-full bg-[#F3F2EE] border border-[#E8E6E0] rounded-xl px-4 py-3 text-[#1A1A1A] text-sm focus:border-amber-500 focus:outline-none transition appearance-none">
                ...
                <div>
                  <label className="text-[10px] text-[#6B6B6B] font-black uppercase tracking-widest block mb-2">A short note (Optional)</label>
                  <textarea rows={3} className="w-full bg-[#F3F2EE] border border-[#E8E6E0] rounded-xl px-4 py-3 text-[#1A1A1A] text-sm focus:border-amber-500 focus:outline-none transition resize-none custom-scrollbar" placeholder="Hi Priya, I am a 3rd year student and would love your advice on..."></textarea>
                </div>
                <button 
                  onClick={() => { alert("Request Sent!"); setSelectedAlumni(null); }}
                  className="w-full py-4 bg-gradient-to-r from-[#C8922A] to-[#D4A843] text-black rounded-xl text-xs font-black uppercase tracking-widest shadow-lg shadow-amber-500/20"
                >
                  Send Request
                </button>
              </div>
