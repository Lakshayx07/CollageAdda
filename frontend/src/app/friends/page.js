    ...
    <Suspense fallback={null}>
      <div className="page-shell relative overflow-x-hidden">
        ...
        <div className="mx-auto flex max-w-[1180px] flex-col items-start gap-8 px-4 py-6 relative z-10 sm:px-6 sm:py-8 md:flex-row md:justify-between lg:gap-16 xl:gap-24">
          ...
          <div className="hidden w-full shrink-0 space-y-6 sticky top-24 md:flex md:w-[320px] md:flex-col xl:w-[360px]">
            ...
            {/* Community Section */}
            <div className="bg-white rounded-[24px] shadow-[0_4px_24px_rgba(0,0,0,0.02)] border border-[#E8E6E0] p-6 mb-8">
              {/* Header */}
              <div className="flex items-center justify-between mb-6">
                <h4 className="text-[15px] font-black text-[#1A1A1A] uppercase tracking-wider flex items-center gap-2">
                ...
                <button onClick={() => router.push('/community')} className="text-[12px] font-black text-[#1A1A1A] flex items-center gap-0.5 hover:opacity-70 transition-opacity cursor-pointer">
                  View All <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg>
                </button>
              </div>
              ...
              <div className="bg-[#FFFDF9] border border-[#F5E6C4] rounded-[20px] p-5 mb-8 relative overflow-hidden">
                ...
                <div className="flex gap-4 items-center relative z-10 mb-5">
                  ...
                  <div>
                    <h5 className="font-black text-[18px] text-[#1A1A1A] leading-tight mb-1">Join the Community</h5>
                    <p className="text-[13px] font-medium text-[#6B6B6B] leading-snug">Connect with students<br />across all campuses.</p>
                  </div>
                </div>
                ...
                <motion.button
                  ...
                  className="w-full bg-gradient-to-r from-[#DE9F35] to-[#C77C1E] text-[#1A1A1A] py-3.5 rounded-[12px] text-[13px] font-black uppercase tracking-wider shadow-[0_4px_12px_rgba(200,146,42,0.3)] flex items-center justify-center gap-2 cursor-pointer transition-all relative z-10"
                >
              ...
              {/* Your Communities Subheader */}
              <div className="flex items-center justify-between mb-4">
                <h4 className="text-[12px] font-black text-[#888888] uppercase tracking-[0.15em]">YOUR COMMUNITIES</h4>
                <span className="text-[11px] font-bold text-[#1A1A1A] bg-[#F9F8F5] border border-[#E8E6E0] px-3 py-1 rounded-full">
                  {Array.from(membershipSet).length} Joined
                </span>
              </div>
                  ...
                  {communities.length > 0 ? communities.map((comm, idx) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="flex items-start gap-2.5 bg-white border border-[#E8E6E0] p-3 rounded-[16px] hover:shadow-sm transition-all group"
                      >
                        {/* Avatar */}
                        <button
                          onClick={() => router.push(`/community/${comm.id}`)}
                          className={`w-[44px] h-[44px] rounded-[14px] ${solidBg} flex items-center justify-center text-[#1A1A1A] font-black text-xl shrink-0 shadow-sm hover:scale-105 transition-transform cursor-pointer mt-0.5`}
                        >
                          {comm.name.charAt(0).toUpperCase()}
                        </button>
                        {/* Info */}
                        <div className="flex-1 min-w-0 text-left flex flex-col justify-center">
                          <button onClick={() => router.push(`/community/${comm.id}`)} className="cursor-pointer text-left w-full">
                            <p className="text-[14px] font-black text-[#1A1A1A] truncate leading-tight mb-0.5">{comm.name}</p>
                            <span className="text-[11px] text-[#888888] font-medium block mb-1.5 whitespace-nowrap">{comm.member_count} members</span>
                          </button>
                          <div className="flex flex-wrap gap-1">
                            {(comm.tags || []).slice(0, 2).map(tag => (
                              <span key={tag} className="text-[9px] bg-[#F9F8F5] border border-[#E8E6E0] text-[#1A1A1A] px-2 py-0.5 rounded-full font-bold truncate max-w-[60px]">#{tag}</span>
                            ))}
                          </div>
                        </div>
                        {/* Join / Joined & 3-dots */}
                    ...
                    <p className="text-[13px] text-[#888888] text-center py-4 bg-white border border-[#E8E6E0] rounded-[16px]">No communities yet — be the first!</p>
                  )}
                </AnimatePresence>
              </div>
          ...
          {selectedProfileId && (
            ...
            >
              ...
              >
                ...
                {/* Close button */}
                <button onClick={() => setSelectedProfileId(null)} className="absolute top-4 right-4 z-20 p-2 bg-white rounded-full text-[#1A1A1A] hover:rotate-90 transition-all duration-300 shadow-[0_4px_12px_rgba(0,0,0,0.1)]">
                    ...
                    <div className="px-6 relative flex-1 flex flex-col">
                      ...
                      {selectedProfileData.isVerified && (
                        <motion.div variants={modalVars.slideUp2} className="flex justify-center mt-4 mb-2">
                          <span className="bg-white text-green-500 text-[11px] font-black uppercase tracking-wide px-4 py-1 rounded-full border-2 border-[#E8E6E0] shadow-sm flex items-center justify-center hover:scale-105 transition-transform cursor-default" style={{ color: '#22c55e' }}>
                            {selectedProfileData.badgeTitle?.toUpperCase().replace(' STUDENT', '') || "VERIFIED"}
                          </span>
                        </motion.div>
                      )}
                      ...
                      <div className="text-center space-y-1.5">
                        ...
                        <motion.p variants={modalVars.fade1} className="text-[11px] font-bold text-[#6B6B6B] mt-2.5 uppercase tracking-wider">
                      ...
                      {/* Stats Row */}
                      <motion.div variants={modalVars.statsContainer} className="grid grid-cols-4 gap-3 mt-6">
                        <motion.div variants={modalVars.statCard} whileHover={{ y: -3, boxShadow: '0 8px 24px rgba(0,0,0,0.06)' }} className="bg-white border border-[#E8E6E0] py-4 px-2 rounded-[20px] text-center flex flex-col items-center justify-center transition-all cursor-default">
                          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-[#1A1A1A] mb-2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>
                          <p className="text-[17px] font-black text-[#1A1A1A]">{selectedProfileData.postsCount !== undefined ? selectedProfileData.postsCount : "—"}</p>
                          <p className="text-[10px] text-[#6B6B6B] font-medium mt-0.5 capitalize">Posts</p>
                        </motion.div>
                        <motion.div variants={modalVars.statCard} whileHover={{ y: -3, boxShadow: '0 8px 24px rgba(0,0,0,0.06)' }} className="bg-white border border-[#E8E6E0] py-4 px-2 rounded-[20px] text-center flex flex-col items-center justify-center transition-all cursor-default">
                          <Users2 size={18} className="text-[#1A1A1A] mb-2" strokeWidth={2.5} />
                          <p className="text-[17px] font-black text-[#1A1A1A]">{(selectedProfileData.followersCount ?? (selectedProfileData.followers !== undefined ? getSocialCount(selectedProfileData.followers) : undefined)) !== undefined ? (selectedProfileData.followersCount ?? getSocialCount(selectedProfileData.followers)) : "—"}</p>
                          <p className="text-[10px] text-[#6B6B6B] font-medium mt-0.5 capitalize">Followers</p>
                        </motion.div>
                        <motion.div variants={modalVars.statCard} whileHover={{ y: -3, boxShadow: '0 8px 24px rgba(0,0,0,0.06)' }} className="bg-white border border-[#E8E6E0] py-4 px-2 rounded-[20px] text-center flex flex-col items-center justify-center transition-all cursor-default">
                          <UserPlus size={18} className="text-[#1A1A1A] mb-2" strokeWidth={2.5} />
                          <p className="text-[17px] font-black text-[#1A1A1A]">{(selectedProfileData.followingCount ?? (selectedProfileData.following !== undefined ? getSocialCount(selectedProfileData.following) : undefined)) !== undefined ? (selectedProfileData.followingCount ?? getSocialCount(selectedProfileData.following)) : "—"}</p>
                          <p className="text-[10px] text-[#6B6B6B] font-medium mt-0.5 capitalize">Following</p>
                        </motion.div>
                        <motion.div variants={modalVars.statCard} whileHover={{ y: -3, boxShadow: '0 8px 24px rgba(0,0,0,0.06)' }} className="bg-white border border-[#E8E6E0] py-4 px-2 rounded-[20px] text-center flex flex-col items-center justify-center transition-all cursor-default">
                          <Zap size={18} className="text-orange-500 mb-2 fill-orange-500" strokeWidth={2.5} />
                          <p className="text-[17px] font-black text-[#1A1A1A]">{getDisplayStreak(selectedProfileData) || 0}</p>
                          <p className="text-[10px] text-[#6B6B6B] font-medium mt-0.5 capitalize">Streak</p>
                        </motion.div>
                      </motion.div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {selectedProfileData.interests.map(int => {
                              const interestEmojis = {
                                'Coding': '💻', 'Music': '🎵', 'Photography': '📸', 'Startups': '🚀', 'Sports': '🏀',
                                'Gaming': '🎮', 'Art': '🎨', 'Reading': '📚', 'Travel': '✈️', 'Design': '✒️',
                                'Writing': '✍️', 'Dance': '💃', 'Movies': '🍿', 'Food': '🍔', 'Fitness': '💪',
                                'Technology': '🤖', 'Fashion': '👗', 'Finance': '💰', 'Business': '💼'
                              };
                              const emoji = interestEmojis[int] || '✨';
                              return (
                                <motion.span
                                  variants={modalVars.interestPill}
                                  whileHover={{ scale: 1.05 }}
                                  key={int}
                                  className="bg-[#FDF9ED] border border-[#F5E6C4] text-[#D49C2C] text-[11px] font-bold px-3 py-1.5 rounded-xl shadow-sm transition-all cursor-default"
                                >
                                  {emoji} {int}
                                </motion.span>
                              );
                      ...
                      {/* Activity Section */}
                      {selectedProfileData.postsCount > 5 && (
                        <motion.div variants={modalVars.activity} className="mt-8">
                          <div className="bg-gradient-to-r from-[#FDF9ED] to-[#FAF3E0] border border-[#F5E6C4] rounded-[24px] p-5 flex items-center gap-5 hover:brightness-95 transition-all cursor-default shadow-[0_2px_12px_rgba(200,146,42,0.06)]">
                            <div className="w-[52px] h-[52px] rounded-2xl bg-[#D49C2C] flex items-center justify-center shrink-0 shadow-sm">
                              <Zap size={24} className="text-white fill-white" />
                            </div>
                            <div className="flex-1">
                              <p className="text-[16px] font-black text-[#1A1A1A]">Frequent Poster</p>
                              <p className="text-[12px] text-[#6B6B6B] font-medium mt-0.5">Regularly posts on campus feed</p>
                            </div>
                          </div>
                        </motion.div>
                      )}
                      ...
                      {/* Action Buttons */}
                      <motion.div variants={modalVars.buttonsContainer} className="mt-8 flex gap-3">
                        {user && (user._id === selectedProfileData._id || user.id === selectedProfileData._id) ? (
                          <>
                            <motion.button variants={modalVars.primaryButton} onClick={() => router.push('/profile')} className="flex-1 py-3.5 bg-white border border-[#E8E6E0] rounded-[20px] text-[13px] font-black text-[#1A1A1A] flex items-center justify-center gap-2 shadow-sm hover:bg-[#F9F8F5] transition-all">
                              ✏️ Edit Profile
                            </motion.button>
                          </>
                        ) : followStatus[selectedProfileData._id] === "connected" ? (
                          <>
                            <motion.button variants={modalVars.primaryButton} onClick={() => { handleDirectMessage(selectedProfileData._id); setSelectedProfileId(null); }} className="flex-1 py-3.5 bg-white border border-[#E8E6E0] rounded-[20px] text-[13px] font-black text-[#1A1A1A] flex items-center justify-center gap-2 shadow-[0_2px_8px_rgba(0,0,0,0.04)] hover:bg-[#F9F8F5] hover:scale-[1.02] transition-all">
                              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"></path><path d="M8 10h.01"></path><path d="M12 10h.01"></path><path d="M16 10h.01"></path></svg> Chat Now
                            </motion.button>
                            <motion.button variants={modalVars.buttonItem} onClick={() => { router.push(`/profile/${selectedProfileData._id}`); setSelectedProfileId(null); }} className="flex-1 py-3.5 bg-white border border-[#E8E6E0] rounded-[20px] text-[13px] font-black text-[#1A1A1A] flex items-center justify-center gap-2 shadow-[0_2px_8px_rgba(0,0,0,0.04)] hover:bg-[#F9F8F5] hover:scale-[1.02] transition-all">
                              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg> View Full Profile
                            </motion.button>
                          </>
                        ) : followStatus[selectedProfileData._id] === "pending" ? (
                          <>
                            <motion.button variants={modalVars.primaryButton} disabled className="flex-1 py-3.5 bg-[#F3F2EE] rounded-[20px] text-[13px] font-black text-[#6B6B6B] border border-[#E8E6E0] flex items-center justify-center gap-2 cursor-not-allowed">
                              ⏳ Request Sent
                            </motion.button>
                            <motion.button variants={modalVars.buttonItem} onClick={() => { router.push(`/profile/${selectedProfileData._id}`); setSelectedProfileId(null); }} className="flex-1 py-3.5 bg-white border border-[#E8E6E0] rounded-[20px] text-[13px] font-black text-[#1A1A1A] flex items-center justify-center gap-2 shadow-[0_2px_8px_rgba(0,0,0,0.04)] hover:bg-[#F9F8F5] hover:scale-[1.02] transition-all">
                              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg> View Full Profile
                          ...
                          <>
                            <motion.button
                              ...
                              className="relative flex-1 py-3.5 bg-blue-500 rounded-[20px] text-[13px] font-black text-white flex items-center justify-center gap-2 shadow-md hover:bg-blue-600 hover:scale-[1.02] transition-all overflow-hidden"
                            >
                              {connectStatus === "idle" && <span className="relative z-10 flex items-center gap-2">⚡ Connect</span>}
                              ...
                              {connectStatus === "connected" && <span className="relative z-10 flex items-center gap-2">✓ Connected!</span>}
                            </motion.button>
                            <motion.button variants={modalVars.buttonItem} onClick={() => { router.push(`/profile/${selectedProfileData._id}`); setSelectedProfileId(null); }} className="flex-1 py-3.5 bg-white border border-[#E8E6E0] rounded-[20px] text-[13px] font-black text-[#1A1A1A] flex items-center justify-center gap-2 shadow-[0_2px_8px_rgba(0,0,0,0.04)] hover:bg-[#F9F8F5] hover:scale-[1.02] transition-all">
                              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg> View Full Profile
                            </motion.button>
