...
export default function PlayerCard({ player }) {
  ...
  const themeColors = isEsports 
    ? "from-cyan-500 via-indigo-600 to-purple-600 shadow-[0_0_30px_rgba(6,182,212,0.5)]" 
  ...
  const SkillBar = ({ label, icon: Icon, value }) => (
    <div className="mb-2 w-full">
      <div className="flex justify-between items-center mb-1">
        <span className="text-[10px] text-white/70 uppercase tracking-widest font-bold flex items-center">
          <Icon size={10} className="mr-1" /> {label}
        </span>
        <span className="text-[10px] font-black text-white">{value}/10</span>
      </div>
      <div className="w-full bg-black/50 rounded-full h-1.5 overflow-hidden border border-white/10">
        <motion.div 
          ...
          className={clsx("h-full rounded-full", isEsports ? "bg-gradient-to-r from-cyan-400 to-purple-500" : "bg-gradient-to-r from-orange-400 to-yellow-400")}
        />
      </div>
    </div>
  );
        ...
        <div className={clsx(
          ...
          <div className="bg-background/90 rounded-xl h-full w-full relative overflow-hidden backdrop-blur-md flex flex-col border border-white/20">
            ...
            <div className="flex justify-between items-center p-3 border-b border-white/10 bg-black/40">
              <span className="text-2xl font-black text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.5)] italic">
                {player.overallRating || 90}
              </span>
              <div className="flex flex-col items-end">
                <span className={clsx("text-[10px] font-black uppercase px-2 py-0.5 rounded border shadow-lg", isEsports ? "bg-cyan-500/20 border-cyan-400/50 text-cyan-300" : "bg-orange-500/20 border-orange-400/50 text-orange-300")}>
                  {player.game_or_sport}
                </span>
                <span className="text-[8px] font-bold text-white/50 uppercase mt-1 tracking-widest">{player.role_or_position}</span>
              </div>
            </div>

            {/* Profile Photo area */}
            <div className="flex-1 flex flex-col items-center justify-center relative p-4">
              <div className={clsx("absolute w-32 h-32 blur-2xl rounded-full", isEsports ? "bg-cyan-500/20" : "bg-orange-500/20")} />
              <div className={clsx(
                "w-24 h-24 rounded-full p-1 bg-gradient-to-b shadow-2xl relative z-10",
                isEsports ? "from-cyan-400 to-purple-600 shadow-cyan-500/50" : "from-orange-400 to-red-600 shadow-orange-500/50"
              )}>
              ...
              <h2 className="text-white font-black text-xl uppercase tracking-wider mt-4 text-center drop-shadow-md">{player.name}</h2>
              <p className="text-xs text-white/60 font-bold tracking-widest">@{player.username}</p>
              <p className="text-[10px] text-white/40 uppercase mt-1">{player.college}</p>
            </div>

            {/* Bottom Mini Stats */}
            <div className="grid grid-cols-3 gap-0 border-t border-white/10 bg-black/60 p-2 text-center relative z-10">
              <div className="border-r border-white/10">
                <span className="block text-[8px] text-white/40 uppercase tracking-widest">Rank</span>
                <span className={clsx("text-xs font-black", isEsports ? "text-cyan-400" : "text-yellow-400")}>{player.rank}</span>
              </div>
              <div className="border-r border-white/10">
                <span className="block text-[8px] text-white/40 uppercase tracking-widest">Available</span>
                <span className="text-[10px] font-bold text-white uppercase mt-0.5 block truncate px-1">{player.availability}</span>
              </div>
              <div>
                <span className="block text-[8px] text-white/40 uppercase tracking-widest">EXP</span>
                <span className="text-[10px] font-bold text-white uppercase mt-0.5 block truncate px-1">{player.experience_level}</span>
              </div>
            </div>
            
            <div className="absolute bottom-16 right-2 animate-bounce opacity-50">
              <span className="text-[8px] text-white uppercase tracking-widest bg-black/50 px-1 py-0.5 rounded">Tap</span>
            </div>
          </div>
        </div>
          ...
          <div className="bg-background/95 rounded-xl h-full w-full relative overflow-hidden backdrop-blur-md flex flex-col border border-white/20 p-4">
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar pr-1">
              ...
              <div className="mt-4 p-2 bg-white/5 rounded-lg border border-white/10">
                <p className="text-[10px] text-white/80 italic text-center">"{player.bio}"</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-4">
              <button 
                ...
                className={clsx("flex flex-col items-center justify-center py-2 rounded-lg border border-white/10 hover:border-white/30 transition shadow-lg", isEsports ? "bg-cyan-500/20 text-cyan-300" : "bg-red-500/20 text-red-300")}
              >
              ...
              <button 
                ...
                className={clsx("flex flex-col items-center justify-center py-2 rounded-lg border border-white/10 hover:border-white/30 transition shadow-lg", isEsports ? "bg-purple-500/20 text-purple-300" : "bg-orange-500/20 text-orange-300")}
              >
              ...
              <button 
                ...
                className="col-span-2 flex items-center justify-center py-2 rounded-lg bg-white/10 border border-white/20 hover:bg-white/20 transition text-white"
              >
