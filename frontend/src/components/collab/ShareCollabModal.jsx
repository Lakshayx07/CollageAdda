        ...
        >
          ...
          <div className="p-4 border-b border-white/10">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-black border border-white/10 rounded-xl py-2.5 pl-9 pr-4 text-sm text-white focus:border-primary focus:outline-none transition"
              />
            </div>
          </div>
          ...
          <div className="overflow-y-auto flex-1 p-2 custom-scrollbar min-h-[200px]">
            {loading ? (
              <div className="flex flex-col items-center justify-center h-40">
                ...
                <p className="text-xs text-white/50 font-bold uppercase tracking-widest">Loading connections...</p>
              </div>
            ) : connections.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-40 text-center px-6">
                <Users size={32} className="text-white/20 mb-3" />
                <p className="text-sm font-bold text-white/50">No friends found</p>
                <p className="text-[10px] text-white/30 mt-1">Follow some users to share cards with them.</p>
              </div>
            ) : filteredConnections.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-40">
                <p className="text-sm font-bold text-white/50">No matches found</p>
              </div>
            ) : (
                ...
                {filteredConnections.map(friend => (
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-3 rounded-xl transition-colors ${
                      selectedFriends.includes(friend._id) ? 'bg-primary/10 border border-primary/20' : 'hover:bg-white/5 border border-transparent'
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <div className="flex items-center gap-1">
                          <p className="text-sm font-bold text-white leading-none">{friend.name}</p>
                          {friend.isVerified && <VerifiedBadge size={12} />}
                        </div>
                        <p className="text-[10px] text-white/50 mt-1">{friend.university || "Student"}</p>
                      </div>
                    </div>
                    <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-colors ${
                      selectedFriends.includes(friend._id) ? 'bg-primary border-primary text-black' : 'border-white/20'
                    }`}>
          ...
          <div className="p-4 border-t border-white/10 shrink-0">
            <button
              ...
              className="w-full py-3.5 bg-gradient-to-r from-primary to-purple-600 hover:from-purple-500 hover:to-blue-600 text-white rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-lg disabled:opacity-50 flex items-center justify-center gap-2"
            >
