...
export function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex flex-col w-64 bg-dark/95 border-r border-white/5 py-6 sticky top-0 h-screen flex-shrink-0 backdrop-blur-xl">
      {/* Logo */}
      <div className="px-6 mb-8">
        <h1 className="text-2xl font-black bg-clip-text text-transparent bg-gradient-to-r from-primary via-secondary to-accent tracking-tighter">
          CollageAdda
        </h1>
        <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-1 opacity-80">Your campus, your vibe 🎓</p>
      </div>
        ...
        {NAV_ITEMS.map(({ to, icon: Icon, label }) => {
          ...
          return (
            <NavLink
              ...
              to={to}
              className={({ isActive }) =>
                `flex items-center space-x-3 px-4 py-3.5 rounded-2xl transition-all text-sm font-semibold group relative
                 ${isActive 
